...
**Identificador único:** PMP-WT-2025-v3.0
...
**Definición:** CMMI (Capability Maturity Model Integration) es un marco de mejora de procesos que orienta cómo desarrollar y mejorar los procesos de ingeniería de software. Se organiza en cinco niveles de madurez y provee buenas prácticas para procesos más eficientes, eficaces y predecibles [14].

**Origen (S22, Diap. 26):** Desarrollado por el SEI (Software Engineering Institute) de Carnegie Mellon en los años 90. Nace como **CMM-SW** para mejorar el software del Departamento de Defensa de EE.UU., basado en el modelo CMM de los años 80; se publica como **CMMI-SE/SW en 2002**. Hoy es uno de los marcos de mejora de procesos más usados en ingeniería de software y otros sectores.
...
**Ventajas de adoptar CMMI (S22, Diap. 36):**

```text
1. Mejora de la calidad de los procesos (menos defectos y repeticiones).
2. Mayor eficacia y productividad.
...
> **Ejemplo aplicado (S22, Diap. 36):** una empresa de banca digital reduce incidentes en producción al identificar cuellos de botella en su proceso de pruebas usando prácticas de CMMI. En WasteTrack, el mismo principio se aplica al rastreo de defectos por módulo y la mejora del pipeline CI/CD (§4.2.6).
...
#### 4.2.8bis Caso ilustrativo 2 — Sistema de préstamos bancarios (referencia externa S22)

El siguiente caso, tomado textualmente de S22 (Diap. 19–23), ilustra la aplicación del ciclo PDCA y los 5 porqués en un dominio distinto al de WasteTrack, reforzando el patrón general del proceso de mejora continua.

**Contexto:** un sistema de préstamos bancarios presenta retrasos en el procesamiento de solicitudes y errores en el cálculo de intereses.

```text
Métrica inicial             Valor
Errores / 100 operaciones   6
Tiempo de respuesta         5.8 s
Exactitud del cálculo       94 %
Satisfacción del cliente    60 %
```

**Análisis con los 5 porqués:**
...
5. ¿Por qué no hay proceso formal de mantenimiento?
   Porque la organización no tiene políticas de calidad que contemplen revisión
   periódica de componentes críticos.

Causa raíz: ausencia de un proceso estandarizado de revisión y mantenimiento
de componentes críticos.
```

**Plan de mejora aplicado:** política de revisión (ISO/IEC 12207), migración a librería matemática certificada, pruebas automáticas de precisión, monitoreo continuo en CI/CD.

**Resultados (antes / después):**

```text
Métrica                    Antes      Después
Errores / 100 operaciones  6          1.5
Exactitud del cálculo      94 %       99.2 %
Tiempo de respuesta        5.8 s      4.1 s
Satisfacción del cliente   60 %       88 %
```

> **Nota:** caso presentado como referencia externa (S22, Diap. 19–23) del ciclo PDCA aplicado en el dominio financiero. En WasteTrack, el patrón equivalente se instancia en §4.2.8 (módulo de rutas S07). Ambos siguen la misma estructura: Contexto → 5 porqués → Causa raíz → Plan PDCA → Métricas antes/después.
...
Documentar los criterios de aceptación ANTES        Ajustar el criterio de aceptación después de
  de medir; congelar las metas al inicio de           ver los resultados para "aprobar" la
  cada sprint o release (S15 §14).                    medición.
```
...
> **Nota de trazabilidad histórica ISO 9126 → ISO/IEC 25010 (S14, Diap. 3–5):**
> ISO 9126 organizaba la calidad en **6 características** (Funcionalidad, Fiabilidad, Usabilidad, Eficiencia, Mantenibilidad, Portabilidad) distribuidas en **4 Partes**:
> - Parte 1 – Modelo de Calidad (características y subcaracterísticas)
> - Parte 2 – Métricas Externas (perspectiva del usuario final)
> - Parte 3 – Métricas Internas (perspectiva del desarrollador)
> - Parte 4 – Métricas de Calidad en Uso (entorno operativo real)
>
> ISO/IEC 25010 amplía ese modelo a 8 características. La tabla siguiente mapea las subcaracterísticas 9126 que no aparecen textualmente en 25010:
>
> ```text
> ISO 9126 (histórica)          ISO/IEC 25010 (vigente)
> Exactitud                     Corrección funcional
> Interoperabilidad             Compatibilidad › Interoperabilidad
> Cumplimiento / Conformidad    (integrado en cada característica)
> Comprensibilidad              Usabilidad › Aprendibilidad + Operabilidad
> Atractivo                     Usabilidad › Estética de la interfaz
> Estabilidad                   Mantenibilidad › Modificabilidad
> ```
>
> Los pesos y métricas del PMP WasteTrack siguen **ISO/IEC 25010** (norma vigente). Esta nota preserva la trazabilidad histórica exigida por el método GQM definido originalmente sobre ISO 9126 [8].
...
Las métricas se organizan según las 8 características de calidad de ISO/IEC 25010 (ISO/IEC 25023:2016 [11]). Tipo de métrica según ISO/IEC 25020 [9]:

- **Directa:** medida objetiva y observable sin cálculo adicional (ej.: tiempo de respuesta en ms).
- **Derivada:** calculada a partir de otras métricas primarias (ej.: densidad de defectos = defectos / KLOC).
- **Subjetiva:** basada en la percepción del usuario, generalmente mediante encuestas (ej.: escala Likert 1–5).

La columna "Resultado real" se completa al cierre de cada sprint / fase.

```text
Característica ISO 25010     Métrica                                  Tipo        Meta               Frecuencia
Adecuación funcional         % de requisitos cubiertos por release    Derivada    ≥ 95 %             Por release
                             Defectos críticos abiertos en entrega    Directa     0                  Por release
Eficiencia de rendimiento    Tiempo de respuesta API / notificación   Directa     ≤ 2 s              Cada sprint
                             Tiempo de carga de pantalla              Directa     ≤ 3 s              Cada sprint
                             Throughput API bajo carga                Directa     ≥ 100 req/s        Sprint 9–10
Compatibilidad               Pruebas en matriz dispositivos           Directa     100 % pass         Sprint 9–10
                             (Android ≥ 8.0, iOS ≥ 13, web)         (pass/fail)
Usabilidad                   Satisfacción de usuario (encuesta)       Subjetiva   ≥ 85 % positivo    Mes 5 (beta)
                             Tasa de éxito en tareas clave            Derivada    ≥ 90 %             Mes 5 (beta)
Fiabilidad                   Disponibilidad del sistema               Directa     ≥ 99 %             Mensual (mes 3+)
                             MTBF (tiempo medio entre fallos)         Derivada    ≥ 720 h            Mes 6 (piloto)
                             MTTR (tiempo medio de recuperación)      Derivada    ≤ 4 h              Mes 6 (piloto)
Seguridad                    Vulnerabilidades críticas OWASP Top 10   Directa     0                  Sprint 8–10
                             Cobertura de análisis estático (SAST)    Directa     100 %              Cada sprint (CI)
Mantenibilidad               Cobertura de pruebas unitarias           Directa     ≥ 80 %             Cada sprint
                             Complejidad ciclomática por función      Directa     ≤ 10               Cada sprint
                             Densidad de defectos                     Derivada    < 5 defectos/KLOC  Cada sprint
Portabilidad                 Tiempo de despliegue automatizado        Directa     ≤ 30 min           Sprint 11
```
...
```text
Característica       Métrica definida                                  Tipo        Meta           Sprint / Fase
Eficacia             % ciudadanos que crean reporte sin asistencia     Derivada    ≥ 90 %         S10 · S12
Productividad        Tiempo promedio de creación de reporte            Directa     ≤ 60 s         S10 · S12
Seguridad en uso     Reportes duplicados o erróneos sin confirmación   Derivada    < 2 %          S10 · S12
Satisfacción         Encuesta post-tarea (escala Likert 1–5)           Subjetiva   ≥ 4.2 / 5      S10 · S12
                     (equivale al ≥ 85 % positivo declarado en OC3)
Contexto de uso      Pruebas en red móvil 3G/4G · Android ≥ 8.0       Directa     100 % pass     S10
                     · iOS ≥ 13 (condiciones declaradas en §1.2)       (pass/fail)
```
...
Documento elaborado bajo los lineamientos de ISO/IEC/IEEE 16326:2009 para el curso Calidad de Software 2025-2. Versión 3.0 — Julio 2026. Incorpora métricas de calidad (ISO/IEC 25000 SQuaRE, GQM, LSP) y proceso de mejora continua (PDCA, CMMI). Versión final tras revisión exhaustiva contra las Sesiones 14, 15, 16 y 22 (tipificación de métricas ISO/IEC 25020, trazabilidad ISO 9126→25010, caso banca digital, origen histórico y ventajas CMMI).
